...
import { getActiveListings, getMyVehicles, getWatchlistIds } from "@/lib/queries";
...
export default async function MarktPage() {
  ...
  const [pool, myVehicles, watchlist] = await Promise.all([
  ...
  return (
    <div>
      ...
      {pool.length === 0 ? (
        ...
        <MarketBrowser pool={pool} myVehicles={myVehicles} watchlist={watchlist} signedIn={Boolean(me)} />
      )}
    </div>
  );
}
